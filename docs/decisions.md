...
| 2026-09-09 | **#6 Staging: ship dist/ to gh-pages as-is — approved whenever Brandon says go.** Recommendation: neither rebuild nor re-scaffold. On-disk `dist/` (372K, base `/demo/`) smoke-passes, matches the committed source on this branch, and is *newer and healthier* than the live `gh-pages` branch — live gh-pages still serves the broken unprocessed 404 template (references `./src/main.js`) and an older asset build. Full inventory + gaps: `docs/dist-inventory.md`. `scripts/stage-gh-pages.sh` is the one-command path (smoke → stage onto local gh-pages → commit as Jack; the coordinator does the push). Nothing pushed by this run; staging deploys wait on Brandon. | Jack |
