...
set -euo pipefail
cd "$(dirname "$0")/.."
if grep -rInE --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=dist -e 'wellacruz' -e 'Brandon Wellacruz' -e 'Captain Brando' . | grep -v '^./scripts/name-audit-check.sh:'; then
