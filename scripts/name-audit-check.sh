...
set -euo pipefail
cd "$(dirname "$0")/.."
if grep -rInE --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=dist -e 'REDACTED-NAME-1' -e 'REDACTED-NAME-2' -e 'REDACTED-NAME-3' . | grep -v '^./scripts/name-audit-check.sh:'; then
